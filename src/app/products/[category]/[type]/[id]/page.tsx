...
export default async function NestedProductPage({
  params,
}: {
  ...
  // Ensure the URL matches the product's assigned category/type
  if (product.category_slug !== category || product.type_slug !== type) {
    notFound()
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
      <div className="mb-6 space-y-3">
        <p className="text-sm text-muted-foreground">
          ...
          <span className="mx-2">/</span>
          <Link href={`/products/${category}/${type}`} className="hover:text-foreground transition-colors">
            {labelFromSlug(type)}
          </Link>
        </p>
